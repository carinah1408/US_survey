...
age <- select(.data = US, age) %>%
summarise(mean_age = mean(age), sd_age = sd(age))
age

gend <- select(.data = US, gend) %>%
   table()
gend

age_gend <- group_by(.data = US, gend) %>%
  select(., age) %>%
  summarise(., mean_age = mean(age), sd_age = sd(age))
age_gend

par_support <- select(.data = US, vote) %>%
   table()
par_support

par_support_gend <- group_by(.data = US, gend) %>%
  select(., vote) %>%
  table()
par_support_gend
...
descriptives2 <- c("cn", "secident", "xeno", "joy", "eff", "pi_1_own", "pi_1_ave", "pi_2_own", "pi_2_ave", "fc_1_s", "fc_1_o_agr", "fc_2_s", "fc_2_o_agr")
...
# correlations (Eta squared, partical correlation, multicollinearity)
...
save_correlation_matrix(df = corr, filename = "Intercorrelation Matrix.csv")
```
