...
# correlation (partical correlation, multicollinearity)
...
US$cn_z <- scale(US$cn)
US$secident_z <- scale(US$secident)
US$xeno_z <- scale(US$xeno)
US$joy_z <- scale(US$joy)
US$eff_z <- scale(US$eff)
US$pi_1_own_z <- scale(US$pi_1_own)
US$pi_1_ave_z <- scale(US$pi_1_ave)
US$pi_2_own_z <- scale(US$pi_2_own)
US$pi_2_ave_z <- scale(US$pi_2_ave)
US$fc_1_s_z <- scale(US$fc_1_s)
US$fc_1_o_agr_z <- scale(US$fc_1_o_agr)
US$fc_2_s_z <- scale(US$fc_2_s)
US$fc_2_o_agr_z <- scale(US$fc_2_o_agr)
US$age_z <- scale(US$age)
US$sd_z <- scale(US$sd)
US$cor_values_z <- scale(US$cor_values)
US$cor_health_z <- scale(US$cor_health)
...
corr <- US %>%
  dplyr::select(., cn_z, secident_z, xeno_z, joy_z, eff_z, pi_1_own_z, pi_1_ave_z, pi_2_own_z, pi_2_ave_z, fc_1_s_z, fc_1_o_agr_z, fc_2_s_z, fc_2_o_agr_z, age_z, sd_z, cor_values_z, cor_health_z)
corr
...
# biserial correlation vote and unexp

US %>%
  dplyr::mutate(vote = as.numeric(vote)) %>% cor.test(~ vote + joy)
...
fc_summary <- US_t1 %>% 
  dplyr::group_by(vote) %>%
  dplyr::summarise(n = n(),
                   mean_fc_1_s = mean(fc_1_s), 
                   se_fc_1_s = sd(fc_1_s)/sqrt(n()), 
                   lower_ci_fc_1_s = mean_fc_1_s - (1.96* se_fc_1_s),
                   upper_ci_fc_1_s = mean_fc_1_s + (1.96* se_fc_1_s),
                   mean_fc_2_s = mean(fc_2_s), 
                   se_fc_2_s =sd(fc_2_s)/sqrt(n()), 
                   lower_ci_fc_2_s = mean_fc_2_s - (1.96* se_fc_2_s),
                   upper_ci_fc_2_s = mean_fc_2_s + (1.96* se_fc_2_s),
                   mean_fc_1_o_agr = mean(fc_1_o_agr), 
                   se_fc_1_o_agr = sd(fc_1_o_agr)/sqrt(n()), 
                   lower_ci_fc_1_o_agr = mean_fc_1_o_agr - (1.96* se_fc_1_o_agr),
                   upper_ci_fc_1_o_agr = mean_fc_1_o_agr + (1.96* se_fc_1_o_agr),
                   mean_fc_2_o_agr = mean(fc_2_o_agr), 
                   se_fc_2_o_agr = sd(fc_2_s)/sqrt(n()),
                   lower_ci_fc_2_o_agr = mean_fc_2_o_agr - (1.96* se_fc_2_o_agr),
                   upper_ci_fc_2_o_agr = mean_fc_2_o_agr + (1.96* se_fc_2_o_agr),
                   )

fc_summary %>%
  ggplot2::ggplot(aes (x = vote, y = mean_fc_2_o_agr)) +
           geom_line(stat="summary", fun = mean, aes(group = NA)) +
           geom_errorbar(aes(ymin = lower_ci_fc_2_o_agr, ymax = upper_ci_fc_2_o_agr), width = .2) +
           geom_point(size = 3, pch = 23, fill = "grey", colour = "grey") +
           scale_x_discrete(name = "Party support") +
           scale_y_continuous(name = "Estimated agreement (%)", limits = c(50, 65)) +
           cowplot::theme_cowplot()

# add coord_flip() for reversed scales
...
# descriptives for behavioural measures

# petition signing

unexp_Rep %>%
  dplyr::select(., time, sign) %>%
  group_by(time, sign) %>%
  summarise(n = n(),
            perc = n/nrow(unexp_Rep)*100) %>%
  kableExtra::kable(col.names = c("*Time*", "*Signed*", "*N*", "*%*"),
                    digits = 2)

# donating

unexp_Rep %>%
  dplyr::select(time, share, donate) %>%
  group_by(time) %>%
  filter(!is.na(share) & !is.na(donate)) %>%
  summarise(mean = mean(share),
            sd = sd(share),
            min = min(share),
            max = max(share),
            mean_don = mean(donate),
            sd_don = sd(donate),
            min_don = min(donate),
            max_don = max(donate)
            )
