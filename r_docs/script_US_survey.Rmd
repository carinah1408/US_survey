...
save_correlation_matrix(df = corr, filename = "Intercorrelation Matrix new.csv")
...
# subset for Democrats (H2)
US_vote_1 <- subset(US_t1, vote == "Democrats")

US_vote_1$time <- factor(US_vote_1$time)
US_vote_1$vote <- factor(US_vote_1$vote)

# subset for Republicans (H2)
US_vote_2 <- subset(US_t1, vote == "Republicans")

US_vote_2$time <- factor(US_vote_2$time)
US_vote_2$vote <- factor(US_vote_2$vote)

# subset Republicans & unexpected (H3 ...)
unexp_Rep <- US %>%
  filter(vote == "Republicans" & unexp == "Unexpected") %>%
  as.tibble()
unexp_Rep
...
unexp_Rep_clean <- US %>%
  filter(vote == "Republicans" & unexp == "Unexpected" & id!= "p160")
unexp_Rep_clean
...
# subset Democrats t1 and t2 (H4)

unexp_Dem <- US %>%
  filter(vote == "Democrats" & unexp == "Unexpected")
unexp_Dem # only 4 participants

US_vote_1a <- subset(US, vote == "Democrats")
...
fc_plot %>%
  ...
  geom_point() +
  labs (x = "Party support", y = "Mean for own approval for harsh measurements against immigrants and refugees") +
  coord_flip() +
  theme_light()
...
fc_plot %>%
  ...
  geom_point() +
  labs (x = "Party support", y = "Mean for agreement estimation with this stance") +
  coord_flip() +
  theme_light()
...
fc_plot %>%
  ...
  geom_point() +
  labs (x = "Party support", y = "Mean for own approval for perception that Whites are unfairly affected by affirmative action") +
  coord_flip() +
  theme_light()
...
```{r, H2, include=TRUE}
# Democrats
pi_1_t_test <- t.test(US_vote_1$pi_1_own, mu = 4.241379)
pi_1_t_test
...
pi_2_t_test <- t.test(US_vote_1$pi_2_own, mu = 5)
pi_2_t_test
...
# Republicans
pi_1_t_testa <- t.test(US_vote_2$pi_1_own, mu = 4.576923)
pi_1_t_testa
...
pi_2_t_testa <- t.test(US_vote_2$pi_2_own, mu = 5.25)
pi_2_t_testa
...
pi_plot <- US_t1 %>%
  ...
  dplyr::group_by(., pi_1_own, pi_1_ave) %>%
  dplyr::summarise(., mean_pi_1_own = mean(pi_1_own), se_pi_1_own = sd(pi_1_own)/sqrt(n()), mean_pi_1_ave = mean(pi_1_ave), se_pi_1_ave = sd(pi_1_ave)/sqrt(n()))
pi_plot


mean_pi_2_own = mean(pi_2_own), se_pi_2_own =sd(pi_2_own)/sqrt(n()), mean_pi_2_ave = mean(pi_2_ave), se_pi_2_ave = sd(pi_2_ave)/sqrt(n()), mean_pi_2_ave = mean(pi_2_ave), se_pi_2_ave = sd(pi_2_ave)/sqrt(n()))
pi_plot %>%
  ggplot2::ggplot(data = ., aes(x = vote, y = mean_pi_1_own)) +
  geom_errorbar(aes(ymin = mean_pi_1_own -2*se_pi_1_own, ymax = mean_pi_1_own +2*se_pi_1_own), width = 0.02) +
  geom_point() +
  labs (x = "Party support", y = "Own comfort with carrying a weapon when living in a diverse neighbourhood") +
  coord_flip() +
  theme_light()

ggplot(US_t1, aes(x=pi_1_own, y = mean, fill=vote))+
  geom_violin(aes(y=..density..),binwidth=1, alpha=.5, position="identity") +
  labs(x= "Own comfort with carrying a weapon when living in a diverse neighbourhood", y= "Frequency") +
  scale_fill_manual(name = "Vote", 
      labels = c("Democratic Party", "Republican Party", NA), 
      values = c("#58a6a6", "#efa355", NA)) +
  theme_light()
...
ggplot(data = US_t1) +
  geom_histogram(aes(x = mean(pi_1_own), y=(..count..)/sum(..count..)), 
                       alpha=0.3, fill ="red",binwidth=2,position="dodge") +
  geom_histogram(aes(x = mean(pi_1_ave), y=(..count..)/sum(..count..)), 
                       alpha=0.3,, fill ="green",binwidth=2,position="dodge")



data <- data.frame(
  comfort=c(rep("Own Comfort" , 10) , rep("Perceived Comfort", 10) ),
  Republicans =rnorm(20,10,4),
  Democrats =rnorm(20,8,3),
)
...
# https://girke.bioinformatics.ucr.edu/GEN242/mydoc_Rgraphics_5.html#bar-plots
US_t1_mean <- aggregate(US_t1[,39:42], by=list(vote=US_t1$vote), FUN=mean)
US_t1_sd <- aggregate(US_t1[,39:42], by=list(vote=US_t1$vote), FUN = sd)

df_mean <- melt(US_t1_mean, id.vars = c("vote"), variable.name = "Comfort with Violence", value.name = "Values")

df_sd <- melt(US_t1_sd, id.vars = c("Party support"), variable.name = "Comfort with violence", value.name = "Values")

limits <- aes(ymax = df_mean[, "Values"] + df_sd[, "Values"], ymin = df_mean[, "Values"] - df_sd[, "Values"])

plot <- ggplot(df_mean, aes(Comfort with Violence, Values, fill = Party support)) +
...
# pi_1_ave
pi_1_paired <- t.test(pi_1_ave ~ time, US_vote_1a, paired = TRUE)
pi_1_paired
...
# pi_2_ave
pi_2_paired <- t.test(pi_2_ave ~ time, US_vote_1a, paired = TRUE)
pi_2_paired
...
# Democrats
# Joy
joy_paired <- t.test(joy ~ time, US_vote_1a, paired = TRUE)
joy_paired
...
# Eff
eff_paired <- t.test(eff ~ time, US_vote_1a, paired = TRUE)
eff_paired
...
# sign all Republicans
sign.data <- data.frame(US_Rep$time, US_Rep$sign) 

sign.data = table(US_Rep$time, US_Rep$sign) 
print(sign.data)

print(chisq.test(sign.data))

# sign unexp
sign_unexp.data <- data.frame(unexp_Rep$time, unexp_Rep$sign) 

sign_unexp.data = table(unexp_Rep$time, unexp_Rep$sign)
print(sign_unexp.data)

print(chisq.test(sign_unexp.data))
...
```{r}
Dem_exp_t1 <- US %>%
  filter(., vote == "Democrats" & unexp == "Expected" & time == "1")
Dem_exp_t1
...
```{r}
subgroup <- US %>%
  filter(., ethn == "White or European American" & xeno >= "5")
subgroup

# no values above 3.25 (above 4 in single items, respectively) for xeno
```

# Collective narcissism and predictions
```{r}
...
# t-tests for overall cn by vote (US = t1 and t2, Democrats & Republicans)
...
# t-test for cn by time
cn2.t.test <- t.test(cn ~ time, data = US_Rep) # US_Rep = US dataset restricted to Republicans only
cn2.t.test

# effect size

t<-cn2.t.test$statistic[[1]]
df<-cn2.t.test$parameter[[1]]
r <- sqrt(t^2/(t^2+df))
round(r, 3)
...
# fc_1_o_agr by cn (controlled for secident) 
fc_cn_model1 <- lm(fc_1_o_agr ~ cn, data = unexp_Rep) # unexp_Rep = US dataset restricted to Republicans that did not expect the outcome, potentially connection between fc and cn here
fc_cn_model1
summary(fc_cn_model1)
coef_lmbeta1 <- lm(fc_cn_model1)
coef_lmbeta1
plot(fc_cn_model1)

fc_cn_model2 <- lm(fc_1_o_agr ~ cn + secident, data = unexp_Rep)
fc_cn_model2
coef_lmbeta2 <- lm(fc_cn_model2)
coef_lmbeta2
summary(fc_cn_model2)

anova(fc_cn_model1, fc_cn_model2)


# fc_2_o_agr by cn (controlled for secident) # uS_Rep since no connection between (un)expectancy and cn is expected here
fc_cn_model3 <- lm(fc_2_o_agr ~ cn, data = US_Rep)
fc_cn_model3
coef_lmbeta3 <- lm(fc_cn_model3)
coef_lmbeta3
summary(fc_cn_model3)
plot(fc_cn_model3)

fc_cn_model4 <- lm(fc_2_o_agr ~ cn + secident, data = US_Rep)
fc_cn_model4
coef_lmbeta4 <- lm(fc_cn_model4)
coef_lmbeta4
summary(fc_cn_model4)

anova(fc_cn_model3, fc_cn_model4)

# sign by cn (logistic regression)
US_vote_2a <- US %>%
  filter(., time == "2" & vote == "Republicans")
US_vote_2a


sign_cn_model <- glm(sign ~ cn + secident, data = US_vote_2a, family = "binomial")
summary(sign_cn_model)
exp(coef(sign_cn_model))
exp(confint(sign_cn_model))
pscl::pR2(sign_cn_model) ["McFadden"] # model has McFadden value of .34 (values > .40 mean that the model fits the data very well), (un)expectancy factor results in increase of .09 in McFadden value

# predictions based on cn-values
sign_cn_new <- data.frame(cn = 7, secident = 1)
predict(sign_cn_model, sign_cn_new, type = "response")

# predictions for each individual in dataset

predicted <- predict(sign_cn_model, US_vote_2a, type = "response")
predicted

# model diagnostics
optimal <- optimalCutoff(US_vote_2a$sign, predicted) [1] 
optimal # any individual with a probability of .79 or higher will be predicted to sign the petition

# sensitivity and specificity
sensitivity(unexp_Rep$sign, predicted)
specificity(unexp_Rep$sign, predicted)

# total miscalculation error rate
misClassError(US_vote_2a$sign, predicted, threshold = optimal) # 10.2 % error rate

# ROC curve
plotROC(US_vote_2a$sign, predicted) # high value with .87 --> model is good in predicting whether or not an individual will sign the petition


# emp by cn 
joy_cn_model1 <- lm(joy ~ cn, data = US_vote_2a)
joy_cn_model1
summary(joy_cn_model1)
coef_lmbeta5 <- lm(joy_cn_model1)
coef_lmbeta5
plot(joy_cn_model1)

joy_cn_clean <- US_vote_2a %>%
  filter(., id!= "p164" & id!= "p122" & id!= "p173")
joy_cn_model2_clean
...
coef_lmbeta6 <- lm(joy_cn_model2)
coef_lmbeta6
plot(joy_cn_model2)

anova(joy_cn_model1, joy_cn_model2)
