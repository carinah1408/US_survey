...
# t-tests for cn and xeno by vote
...
xeno.t.test <- t.test(xeno ~ vote, data = US_t1)
xeno.t.test

# effect size

t<-xeno.t.test$statistic[[1]]
df<-xeno.t.test$parameter[[1]]
r <- sqrt(t^2/(t^2+df))
round(r, 3)
...
cn2.t.test <- t.test(cn ~ time, data = US_Rep)
cn2.t.test
...
fc_cn_model1 <- lm(fc_1_o_agr ~ cn, data = unexp_Rep)
fc_cn_model1
...
# fc_2_o_agr by cn (controlled for secident)
...
sign_cn_model <- glm(sign ~ cn + secident, data = unexp_Rep, family = "binomial")
summary(sign_cn_model)
pscl::pR2(sign_cn_model) ["McFadden"] # model has McFadden value of .34 (values > .40 mean that the model fits the data very well)
...
predicted <- predict(sign_cn_model, unexp_Rep, type = "response")
predicted

# model diagnostics
optimal <- optimalCutoff(unexp_Rep$sign, predicted) [1] 
optimal # any individual with a probability of .27 or higher will be predicted to sign the petition
...
misClassError(unexp_Rep$sign, predicted, threshold = optimal) # 10.2 % error rate

# ROC curve
plotROC(unexp_Rep$sign, predicted) # high value with .87 --> model is good in predicting whether or not an individual will sign the petition
