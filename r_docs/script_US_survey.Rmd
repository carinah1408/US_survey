...
# fc_1_o_agr by cn (controlled for secident), unexp_Rep = US dataset restricted to Republicans that did not expect the outcome, potentially connection between fc and cn here
fc_cn_model <- lm(fc_1_o_agr ~ cn + secident, data = unexp_Rep)
fc_cn_model
summary(fc_cn_model)
confint(fc_cn_model)
coef_lmbeta2 <- lm.beta(fc_cn_model)
coef_lmbeta2
...
fc_cn_model4
coef_lmbeta4 <- lm.beta(fc_cn_model4)
coef_lmbeta4
summary(fc_cn_model4)

# sign by cn (logistic regression)
sign_cn_model <- glm(sign ~ cn + secident, data = US_Rep, family = "binomial")
...
car::vif(sign_cn_model) # no multicollinearity


# predictions based on cn-values
sign_cn_new <- data.frame(cn = 4, secident = 5)
predict(sign_cn_model, sign_cn_new, type = "response")

# predictions for each individual in dataset

predicted <- predict(sign_cn_model, US_Rep, type = "response")
predicted

# model diagnostics
optimal <- optimalCutoff(US_Rep$sign, predicted) [1] 
optimal # any individual with a probability of .31 or higher will be predicted to sign the petition

# sensitivity and specificity
sensitivity(US_Rep$sign, predicted)
specificity(US_Rep$sign, predicted)

# total miscalculation error rate
misClassError(US_Rep$sign, predicted, threshold = optimal) # 12 % error rate

# ROC curve
plotROC(US_Rep$sign, predicted) # high value with .81 --> model is ok in predicting whether or not an individual will sign the petition

# sharing by cn
share_cn_model1 <- lm(share ~ cn + secident, data = US_Rep)
share_cn_model1
summary(share_cn_model1)

coef_lmbeta5 <- lm(share_cn_model1)
coef_lmbeta5
plot(share_cn_model1)
...
eff_cn_model1 <- lm(eff ~ cn, data = US_Rep)
eff_cn_model1
summary(eff_cn_model1)

eff_cn_model2 <- lm(eff ~ cn + secident, data = US_Rep)
eff_cn_model2
summary(eff_cn_model2)
confint(eff_cn_model2)
plot(eff_cn_model2)

anova(eff_cn_model1, eff_cn_model2)
