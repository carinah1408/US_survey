...
# creating new variables (cn, secident, xeno, joy & eff) and assumption checks
...
```{r, new variables and assumption checks, include=TRUE}
US <- US_data %>%
...
descriptives4 <- US %>% 
  group_by(vote, time) %>%
  select(., cn, secident, xeno, joy, eff, pi_1_own, pi_1_ave, pi_2_own, pi_2_ave, fc_1_s, fc_1_o_agr, fc_2_s, fc_2_o_agr) %>%
  summarise(., mean_cn = mean(cn), sd_cn = sd(cn), 
            mean_secident = mean(secident), sd_secident = sd(secident), 
            mean_xeno = mean(xeno), sd_xeno = sd(xeno),
            mean_joy = mean(joy), sd_joy = sd(joy),
...
# f_1_s
f_1_s_.t.test <- t.test(fc_1_s ~ vote, data = US)
f_1_s_.t.test
...
# f_1_o_agr
f_1_o_agr.t.test <- t.test(fc_1_o_agr ~ vote, data = US)
f_1_o_agr.t.test
...
# f_2_s
f_2_s_.t.test <- t.test(fc_2_s ~ vote, data = US)
f_2_s_.t.test
...
f_2_o_agr.t.test <- t.test(fc_2_o_agr ~ vote, data = US)
f_2_o_agr.t.test
...
```{r}
# Democrats
US_vote_1 <- subset(US, vote == "Democrats")

pi_1_t_test <- t.test(US_vote_1$pi_1_own, US_vote_1$pi_1_ave)
pi_1_t_test
...
pi_2_t_test <- t.test(US_vote_1$pi_2_own, US_vote_1$pi_2_ave)
pi_2_t_test
...
# Republicans
US_vote_2 <- subset(US, vote == "Republicans")

pi_1_t_testa <- t.test(US_vote_2$pi_1_own, US_vote_2$pi_1_ave)
pi_1_t_testa
...
pi_2_t_testa <- t.test(US_vote_2$pi_2_own, US_vote_2$pi_2_ave)
pi_2_t_testa
